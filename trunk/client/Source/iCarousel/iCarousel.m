...
@property (nonatomic, AH_WEAK) id timer;
...
/**
 * Returns the view containing the given point (which must be in carousel space) or NULL.
 */
- (NSView *)viewAtPoint: (NSPoint)point
{
    CALayer *layer = [contentView.layer hitTest: NSPointToCGPoint(point)];
    for (NSView *view in itemViews.allValues)
    {
        if (view.layer == layer)
            return view;
    }
...
- (void)mouseDragged:(NSEvent *)theEvent
{
    if (scrollEnabled)
    {
        ...
        translation *= useY ? [theEvent deltaY] : [theEvent deltaX];
        
        CGFloat factor = 1; 
        if (!shouldWrap && bounces)
        {
            factor -= fminf(fabsf(scrollOffset - [self clampedOffset:scrollOffset]) / itemWidth, bounceDistance) / bounceDistance;
        }
...
- (void)scrollWheel:(NSEvent *)theEvent
{
    ...
    int offset = [theEvent deltaY] > 0 ? -1 : 1;
    [self scrollToItemAtIndex:self.currentItemIndex + offset animated:YES];
}
