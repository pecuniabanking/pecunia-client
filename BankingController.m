...
-(BankAccount*)selectedBankAccount
{
	...
	if([cat isMemberOfClass: [Category class ] ]) {
        [[MessageLog log ] addMessage:@"selectedBankAccount: Kategorie selektiert!" withLevel: LogLevel_Debug ];
        return nil;
    }	
	NSString *accNumber = [cat valueForKey: @"accountNumber" ];
	if(accNumber == nil || [accNumber isEqual: @"" ]) {
        [[MessageLog log ] addMessage:@"selectedBankAccount: Kontonummer leer!" withLevel: LogLevel_Debug ];
        return nil;        
    }
	return (BankAccount*)cat;
}

-(BankAccount*)checkForTransfer
{
	BankAccount* account = [self selectedBankAccount ];
	if(account == nil) return nil;
	if ([[account isManual ] boolValue] == YES) {
        [[MessageLog log ] addMessage:@"checkForTransfer: Konto wird manuell gepflegt!" withLevel: LogLevel_Debug ];
        return nil;
    }
    if (transferWindowController == nil) {
        [[MessageLog log ] addMessage:@"checkForTransfer: Transferwindowcontroller ist nicht vorhanden!" withLevel: LogLevel_Debug ];
        return nil;
    }
    return account;
}
...
{
	BankAccount* account = [self checkForTransfer ];
