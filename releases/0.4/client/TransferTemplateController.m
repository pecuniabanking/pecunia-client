...
-(BOOL)selectionShouldChangeInTableView:(NSTableView *)tView
{
    NSArray *sel = [templateController selectedObjects ];
	...
	TransferTemplate *template = [sel lastObject ];
    if (skipMessage == YES) return NO;
    if([self checkTemplate:template ] == YES) {
        return YES;
    } else {
        skipMessage = YES;
        [self performSelector:@selector(resetSkip) withObject:nil afterDelay:0.0 ];
        return NO;
    }
}
