...
{
    NSMutableString *s = [NSMutableString stringWithFormat:@"Datum: %@, Wert: %@\n", [[ShortDate dateWithDate: self.date ] description ], self.value ];
    NSString *x = nil;
    if (self.purpose) {
        x = [self.purpose stringByReplacingOccurrencesOfString:@" " withString:@"_" ];
        [s appendFormat:@"Verwendungszweck: %@\n", x ];
    }
    if (self.remoteName) {
        x = [self.remoteName stringByReplacingOccurrencesOfString:@" " withString:@"_" ];
        [s appendFormat:@"Name: %@\n", x ];
    }
    [s appendFormat:@"Konto: %@, Bankleitzahl: %@\n", self.remoteAccount, self.remoteBankCode ];
    [s appendFormat:@"IBAN: %@, BIC: %@", self.remoteIBAN, self.remoteBIC ];
...
-(BOOL)matches: (BankStatement*)stat
{
    ...
    BOOL debug = [defaults boolForKey:@"debugStatementMatching" ];
    
    if (debug) {
        log = [MessageLog log ];
        [log addMessage:[NSString stringWithFormat:@"Umsatz 1:\n\n%@Umsatz 2:\n%@", [self debugStringValue ], [stat debugStringValue ] ] withLevel:LogLevel_Debug ];
    }
	...
	if ([d1 isEqual: d2 ] == NO) {
        if (debug) [log addMessage:@"Datum ungleich" withLevel:LogLevel_Debug ];
        return NO;
    }
    
	NSDecimalNumber *d = [[self.value decimalNumberBySubtracting: stat.value ] abs ];
	if ([d compare:e ] == NSOrderedDescending) {
        if (debug) [log addMessage:@"Betrag ungleich" withLevel:LogLevel_Debug ];
        return NO;
    }
	
	if (stringEqualIgnoreWhitespace(self.purpose, stat.purpose) == NO) {
        if (debug) [log addMessage:@"Verwendungszweck ungleich" withLevel:LogLevel_Debug ];
        return NO;        
    }
    
	if (stringEqualIgnoreWhitespace(self.remoteName, stat.remoteName) == NO) {
        if (debug) [log addMessage:@"Name ungleich" withLevel:LogLevel_Debug ];
        return NO;
    }

	if (stringEqualIgnoringMissing(self.remoteAccount, stat.remoteAccount) == NO) {
        if (debug) [log addMessage:@"Konto ungleich" withLevel:LogLevel_Debug ];
        return NO;
    }
    
	if (stringEqualIgnoringMissing(self.remoteBankCode, stat.remoteBankCode) == NO) {
        if (debug) [log addMessage:@"BLZ ungleich" withLevel:LogLevel_Debug ];
        return NO;
    }
    
	if (stringEqualIgnoringMissing(self.remoteBIC, stat.remoteBIC) == NO) {
        if (debug) [log addMessage:@"BIC ungleich" withLevel:LogLevel_Debug ];
        return NO;
    }
    
	if (stringEqualIgnoringMissing(self.remoteIBAN, stat.remoteIBAN) == NO) {
        if (debug) [log addMessage:@"IBAN ungleich" withLevel:LogLevel_Debug ];
        return NO;
    }
    
    if (debug) [log addMessage:@"Umsätze sind gleich." withLevel:LogLevel_Debug ];
	return YES; 
}
