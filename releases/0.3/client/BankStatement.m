...
*/ 
	...
	if (stringEqual(self.remoteAccount, stat.remoteAccount) == NO) return NO;
	if (stringEqual(self.remoteBankCode, stat.remoteBankCode) == NO) return NO;
	if (stringEqual(self.remoteBIC, stat.remoteBIC) == NO) return NO;
	if (stringEqual(self.remoteIBAN, stat.remoteIBAN) == NO) return NO;
	return YES; 
}
...
#ifdef DEBUG_DUPL_CHECK
        NSLog(@"Check: %@ <> %@", self.purpose, stat.purpose);
#endif
        return NO;
    }
	if (stringEqualIgnoreWhitespace(self.remoteName, stat.remoteName) == NO) {
#ifdef DEBUG_DUPL_CHECK
        NSLog(@"Check: %@ <> %@", self.remoteName, stat.remoteName);
#endif
        return NO;        
    }
	
	if (stringEqual(self.remoteAccount, stat.remoteAccount) == NO) {
#ifdef DEBUG_DUPL_CHECK
        NSLog(@"Check: %@ <> %@", self.remoteAccount, stat.remoteAccount);
#endif
        return NO;                
    }
    
	if (stringEqual(self.remoteBankCode, stat.remoteBankCode) == NO) {
#ifdef DEBUG_DUPL_CHECK
        NSLog(@"Check: %@ <> %@", self.remoteBankCode, stat.remoteBankCode);
#endif
        return NO;        
    }
    
	if (stringEqual(self.remoteBIC, stat.remoteBIC) == NO) {
#ifdef DEBUG_DUPL_CHECK
        NSLog(@"Check: %@ <> %@", self.remoteBIC, stat.remoteBIC);
#endif
        return NO;        
    }
    
	if (stringEqual(self.remoteIBAN, stat.remoteIBAN) == NO) {
#ifdef DEBUG_DUPL_CHECK
        NSLog(@"Check: %@ <> %@", self.remoteIBAN, stat.remoteIBAN);
#endif
        return NO;                
    }
...
#ifdef DEBUG_DUPL_CHECK
        NSLog(@"Check: %@ <> %@", self.value, stat.value);
#endif
        return NO;                
    }
