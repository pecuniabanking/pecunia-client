...
-(IBAction)enqueueRequest: (id)sender
{
	...
	if(cat.accountNumber != nil) [selectedAccounts addObject: cat ];
		...
		for(account in selectedNodes) {
			NSArray *result;
			NSPredicate *predicate = [NSPredicate predicateWithFormat: @"parent = %@ AND noAutomaticQuery = 0 AND isManual = 0", account ];
			[request setPredicate:predicate];
...
-(void)migrate
{
	...
	while (allChecked == NO) {
		...
		for(NSToolbarItem *item in items) {
			...
			idx++;
			if ([identifier isEqualToString:@"catHistory" ] == NO && [identifier isEqualToString: @"catPeriods" ] == NO && [identifier isEqualToString: @"standingOrders" ] == NO) continue;
			for(i=idx+1; i< [items count ]; i++) {
				NSToolbarItem *item2 = [items objectAtIndex:i ];
				if ([identifier isEqualToString: [item2 itemIdentifier ] ]) {
