...
- (id)initWithFrame: (NSRect)frame {
    ...
    if (self != nil) {
        if (![NSBundle.mainBundle loadNibNamed: @"HomeScreenTransfersView" owner: self topLevelObjects: nil]) {
            LogError(@"Interner Fehler: home screen recent transfers view loading failed.");
        }
...
- (void)handleDataModelChange: (NSNotification *)notification {
    ...
    @try {
        ...
        if (insertedObjects.count > 0 || deletedObjects.count > 0) {
            ...
            NSMutableArray *newEntries = [[BankingCategory.bankRoot allAssignmentsOrderedBy: DateOrderDate
                                                                                      limit: 50
                                                                                  recursive: YES
...
- (void)loadData {
    ...
    entries = [[BankingCategory.bankRoot allAssignmentsOrderedBy: DateOrderDate
                                                           limit: 50
                                                       recursive: YES
                                                       ascending: NO] mutableCopy];
