...
- (void)scrollPageUp:(id)sender
{
  ...
  [[self contentView] scrollToPoint: visibleRect.origin];
...
- (void)scrollPageDown:(id)sender
{
  NSRect visibleRect = [self documentVisibleRect];
  visibleRect.origin.y += visibleRect.size.height;
  if (NSMaxY(visibleRect) > _totalHeight)
    visibleRect.origin.y -= NSMaxY(visibleRect) - _totalHeight;
...
- (BOOL)attemptDragWithMouseDown:(NSEvent*)theEvent inCell:(PXListViewCell*)theCell
{
  ...
  if (dragResult == PXIsDragStartMouseMovedHorizontally || dragResult == PXIsDragStartMouseMovedVertically)
	{
...
#pragma unused(dragEvent)
    ...
    for( PXListViewCell* currCell in _visibleCells )
    {
        ...
        { 
            ...
            bounds.origin.x = rowRect.origin.x -minX;
            bounds.origin.y = imageSize.height - (rowRect.origin.y - minY + rowRect.size.height);	// Document view is flipped, so flip the coordinates before drawing into image, or the list items will be reversed.
