...
- (void)loadWindow {
    ...
    self->year = [calendar component: NSCalendarUnitYear fromDate:date];
...
- (void)selectData {
    ...
    ShortDate *start = [ShortDate dateWithYear:(unsigned)year month:1 day:1];
    ShortDate *end = [ShortDate dateWithYear:(unsigned)year month:12 day:31];
    ...
    end = [ShortDate dateWithYear:(unsigned)year month:month+1 day:1];
    endDate = [end lowDate];
    for (BankStatement *statement in statements) {
        ...
        if ([statement.date compare: endDate] != NSOrderedAscending) {
            month++;
            if (month == 13) {
                end = [ShortDate dateWithYear:(unsigned)year month:12 day:31];
